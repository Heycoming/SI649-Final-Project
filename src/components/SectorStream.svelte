<script>
  ...
  let hoverData = $state(null); // 存储时间维度的数据 (用于 Tooltip)
  let hoveredCategory = $state(null); // 存储当前悬浮的类别名称 (用于高亮)
  ...
  let processedData = $derived.by(() => {
    ...
    const categories = Array.from(new Set(rawData.map(d => d["Big Category"])));
    const grouped = d3.group(rawData, d => `${d.Year}-${String(d.Month).padStart(2, '0')}`);
  ...
  let scales = $derived.by(() => {
    ...
    const color = d3.scaleOrdinal()
      .domain(categories)
      .range([
        "#4FC3F7", "#FFB74D", "#EF5350", "#BA68C8", "#81C784",
        "#FFD54F", "#4DB6AC", "#A1887F", "#90A4AE", "#7986CB"
      ]);
...
<div class="wrapper">
  ...
  <div class="chart-container" bind:clientWidth={width}>
    ...
    <p class="chart-subtitle">Evolution of expenditure categories over time (2020-2024)</p>
    ...
    {#if scales.series}
      ...
      >
        <g>
          {#each scales.series as layer}
            <path
              ...
              stroke={hoveredCategory === layer.key ? "#fff" : "rgba(255,255,255,0.05)"}
        ...
        <g transform="translate(0, {height - margin.bottom})">
          <line x1={margin.left} x2={width - margin.right} stroke="#fff" stroke-opacity="0.1" />
          {#each scales.x.ticks(width > 600 ? 10 : 5) as tick}
            <g transform="translate({scales.x(tick)}, 0)">
              <line y2="6" stroke="#fff" stroke-opacity="0.3" />
              <text y="20" fill="#aaa" font-size="10" text-anchor="middle" font-family="JetBrains Mono">
      ...
      {#if hoverData}
        ...
        >
          ...
          <div class="tooltip-body">
            {#each processedData.categories.slice().sort((a,b) => hoverData[b] - hoverData[a]) as cat}
              <div 
                  class="tooltip-row"
                  style:opacity={hoveredCategory && hoveredCategory !== cat ? 0.3 : 1}
              >
                <span class="row-color" style:background={scales.color(cat)}></span>
  ...
  <article class="narrative-section">
    <h3 class="narrative-title">The Money Cycle: How Industries Spend on US Elections</h3>
    <div class="narrative-content">
      <p>
        The chart above tells a clear story: political spending moves in waves. A huge spike is visible in <strong>late 2022</strong>. This demonstrates how much money poured in during the Midterm Elections, when control of Congress was at stake.
      </p>
      <!-- 修改点 1：将 Corporations 统一改为 Business related -->
      <p>
        <strong>Why is "Business related" the biggest?</strong> The blue layer represents <strong>business related</strong> interests, and it is always the largest. <strong>Large companies and trade groups</strong> spend money steadily to protect their profits. They want favorable tax laws and stable rules, so they keep donating year-round, not just during election season.
      </p>
      <p>
        In contrast, other groups like <strong>Non-Workforce</strong> (the red layer) tend to react to big moments. Their spending jumps up when voters are angry or excited about specific issues. As the 2024 election approaches, the stream is widening again, indicating that the race is heating up.
      </p>
    </div>
  </article>
  ...
  <section class="conclusion-section">
    ...
    <div class="conclusion-text">
      <p>
        This investigation into Michigan's campaign finance data reveals a complex truth about <strong>the democratic system</strong>. While elections are often framed as a contest of ideas, the data shows they are also a contest of resources. From the tech hubs of California to the lobbying firms of D.C., money flows into Michigan because the outcome here shapes the future of the entire nation.
      </p>
      <!-- 修改点 2：将 Corporate donors 改为 Business related groups -->
      <p>
        <strong>Business related</strong> groups provide the steady baseline of funding, ensuring their interests are always protected. Meanwhile, ideological groups create the sudden waves of cash that define the most heated moments of the campaign.
      </p>
    ...
    <div class="footer-note">
      <p>© 2024 Michigan Money Trail Investigation. Data provided by the Bureau of Elections.</p>
    </div>
  </section>
...
<style>
  ...
  .row-color { width: 8px; height: 8px; border-radius: 50%; margin-right: 6px; }
  .row-label { flex: 1; opacity: 0.8; font-size: 0.75rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .row-val { font-family: 'JetBrains Mono', monospace; font-weight: bold; }
  .tooltip-total {
    margin-top: 0.5rem;
    padding-top: 0.5rem;
