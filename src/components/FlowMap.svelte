<script>
	...
	let alluvialData = $derived.by(() => {
        ...
        if (showAll) {
            ...
            const topN = 10;
            const topFunders = [...miCountyData].sort((a, b) => b.total_amount - a.total_amount).slice(0, topN).map((d) => d.county);
            const topSpenders = [...expenditureData].map((d) => ({ ...d, total_amount: +d.total_amount })).sort((a, b) => b.total_amount - a.total_amount).slice(0, topN).map((d) => d.county);
            unionSet = new Set([...topFunders, ...topSpenders]);
        }

		let leftData = [...miCountyData].filter((d) => unionSet.has(d.county)).map((d) => ({ ...d, val: d.total_amount })).sort((a, b) => b.val - a.val);
		let rightData = [...expenditureData].filter((d) => unionSet.has(d.county)).map((d) => ({ ...d, val: +d.total_amount })).sort((a, b) => b.val - a.val);
		...
		const xLeftCol = 220, xPool = 480, xRightCol = 740, colWidth = 10;

		const knotHeight = 50;
		const knotTop = margin.top + (height - knotHeight) / 2;
		const knotScale = d3.scaleLinear().domain([0, maxTotal]).range([0, knotHeight]);
		...
		let leftItems = leftData.map((d) => {
			...
			const item = { ...d, x: xLeftCol, y: yCursor, h: h, knotY: knotCursorLeft, knotH: hKnot, color: color, barLen: barScale(d.val) };
			yCursor += h; knotCursorLeft += hKnot;
			return item;
		});
		...
		let rightItems = rightData.map((d) => {
			...
			const hKnot = knotScale(d.val);
			const color = getNetColor(d.county);
			const item = { ...d, x: xRightCol, y: yCursor, h: h, knotY: knotCursorRight, knotH: hKnot, color: color, barLen: barScale(d.val) };
			yCursor += h; knotCursorRight += hKnot;
			return item;
		});

		const leftLinks = leftItems.map((d) => ({
			path: getRibbonPath(d.x + colWidth, d.y, d.h, xPool - 2, d.knotY, d.knotH),
			color: d.color, data: d
		}));
		...
		return { leftItems, rightItems, leftLinks, rightLinks, xLeftCol, xRightCol, xPool, knotTop, knotHeight, colWidth };
	});
	...
	onMount(() => {
		...
		miCountyData.forEach((d) => { if (d.county) countyFundingMap.set(d.county, d.total_amount); });
		expenditureData.forEach((d) => { if (d.county) countyExpMap.set(d.county, +d.total_amount); });
		...
		fundingData.forEach((d) => {
			...
			if (s && t) {
				...
				allNodes.push({
					...d, type: "out-state", homeX: s[0], homeY: s[1], x: s[0], y: s[1],
					r: Math.max(3, r * 0.7), r_compare: r, formattedAmount: d3.format("$,.0f")(d.amount), color: nodeColor
				});
				...
				processedArcs.push({
					...
					color: nodeColor, strokeWidth: wScale(d.amount)
				});
			}
		});
		...
		miCountyData.forEach((d) => {
			...
			if (c) {
				...
				allNodes.push({
					...d, type: "in-state", homeX: c[0], homeY: c[1], x: c[0], y: c[1],
					r_map: Math.max(2, r * 0.15), r_compare: r, formattedAmount: d3.format("$,.0f")(d.total_amount),
		...
		expenditureData.forEach((d) => {
			...
			if (c) {
				const r = rScale(d.total_amount);
				allNodes.push({
					...d, type: "expenditure", homeX: c[0], homeY: c[1], x: c[0], y: c[1],
					r_map: Math.max(2, r * 0.15), r_compare: r, formattedAmount: d3.format("$,.0f")(d.total_amount),
		...
		nodes = allNodes; arcs = processedArcs;
		simulation = d3.forceSimulation(nodes).alphaDecay(0.02).on("tick", () => { nodes = [...nodes]; });
		isReady = true;
	});
	...
	$effect(() => {
		...
		if (step <= 2) { 
			...
			simulation.force("x", d3.forceX((d) => d.type === "out-state" ? centerLeft.x : centerRight.x).strength(0.08))
				.force("y", d3.forceY((d) => d.type === "out-state" ? centerLeft.y : centerRight.y).strength(0.08)).force("collide", collide);
		} else if (step === 4) { 
			simulation.force("x", d3.forceX((d) => { if (d.type === "out-state") return centerLeft.x; return d.homeX; }).strength((d) => (d.type === "out-state" ? 0.08 : 0.8)))
	...
	function getNodeOpacity(n, s) {
		...
		if (s === 3) return n.type === "out-state" || n.type === "in-state" ? 0.95 : 0;
    ...
    function getStrokeColor(feature) {
        if (activeHighlight && feature.properties.name === activeHighlight) return "#FFF";
        return "#555";
    }
    function getStrokeWidth(feature) {
        if (activeHighlight && feature.properties.name === activeHighlight) return 2;
        return 0.5;
    }
    ...
    function handleMapHover(e, feature, type) {
        if (step === 4) {
             let name = feature.properties.name;
             let val = type === "funding" ? countyFundingMap.get(name) : countyExpMap.get(name);
             hoveredNode = { 
                 county: name, 
...
<div class="map-container" bind:clientWidth={containerWidth} bind:clientHeight={containerHeight}>
	...
	<svg width="100%" height="100%" viewBox="0 0 {baseWidth} {baseHeight}" preserveAspectRatio="xMidYMid meet">
		<g class="zoom-group" style="transform: {transformString};">
			<g class="map-layer" style="opacity: {step === 6 || step === 0 ? 0 : 1}; transition: opacity 0.5s;">
				{#if step < 4}
					{#each usFeatures as f}
						<path d={pathGenerator(f)} fill="none" stroke="#444" stroke-width="1" vector-effect="non-scaling-stroke" style="transition: opacity 1s;" opacity={step === 3 ? 0 : 1} />
					{/each}
				{/if}
				<g class="mi-center" style="transition: opacity 1s; opacity: {step === 4 ? 1 : 0}; pointer-events: {step === 4 ? 'auto' : 'none'};">
                    {#each miFeatures as f}
                        <path d={pathGenerator(f)} fill="none" stroke="#666" stroke-width="0.5" vector-effect="non-scaling-stroke" 
                              style="cursor: pointer; transition: stroke 0.2s;"
                              onmouseenter={(e) => { e.target.setAttribute('stroke', '#FFF'); e.target.setAttribute('stroke-width', '1.5'); handleMapHover(e, f, "funding"); }}
                              onmouseleave={(e) => { e.target.setAttribute('stroke', '#666'); e.target.setAttribute('stroke-width', '0.5'); hoveredNode = null; }}
                        />
                    {/each}
				</g>
				<g class="split-maps" style="transition: opacity 1s; opacity: {step >= 5 ? 1 : 0};">
					<g class="mi-left" style="transform: translate(-{200 / currentScale}px, 0);">
						...
						{/each}
						<text x={miCenter.x} y={miCenter.y - 150} text-anchor="middle" font-size="20" fill="#B39DDB" style="pointer-events: none;">Funding Inflow</text>
					</g>
					<g class="mi-right" style="transform: translate({200 / currentScale}px, 0);">
						{#each miFeatures as f}
							<path d={pathGenerator(f)} 
                                  ...
                                  style="transition: fill 1s, stroke 0.3s;" />
						{/each}
						<text x={miCenter.x} y={miCenter.y - 150} text-anchor="middle" font-size="20" fill="#FFF59D" style="pointer-events: none;">Expenditures</text>
					</g>
				</g>
			</g>
			...
			<g class="particle-layer">
				{#each nodes as n}
					...
					<circle cx={n.x} cy={n.y} r={getCurrentRadius(n, step) / (step >= 4 ? currentScale * 0.8 : 1)} 
            ...
            {#if annotationTarget && step < 5 && step > 0}
                <g class="annotation-layer" style="pointer-events: none;">
                    <circle cx={annotationTarget.x} cy={annotationTarget.y} r={20 / currentScale} fill="none" stroke="#FFD700" stroke-width="2" opacity="0.8">
                        <animate attributeName="r" from={10/currentScale} to={30/currentScale} dur="1.5s" repeatCount="indefinite" />
                        <animate attributeName="opacity" from="1" to="0" dur="1.5s" repeatCount="indefinite" />
                    </circle>
                    <line x1={annotationTarget.x} y1={annotationTarget.y} x2={annotationTarget.x + 40/currentScale} y2={annotationTarget.y - 40/currentScale} stroke="#FFD700" stroke-width="1.5" />
                    <text x={annotationTarget.x + 45/currentScale} y={annotationTarget.y - 40/currentScale} fill="#FFD700" font-size={14/currentScale} font-weight="bold" alignment-baseline="middle">
